...
class StationDataDb(object):
    ...
    def __init__(self, nc_path, startend_ymd=None, vcc_size=None,
        ...
        nc_data = Dataset(nc_path, mode=mode)# "w", format="NETCDF4")
        store = xr.backends.NetCDF4DataStore(nc_data, mode=mode)
        ...
        for a_varname in list(self.ds.variables.keys()):
    ...
    def add_stn_variable(self, varname, long_name, units, dtype, fill_value=None, reset=True):
        ...
        if varname not in list(self.ds.variables.keys()):
