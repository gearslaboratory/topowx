...
from twx.db.station_data import STN_NAME, ELEV, LAT, LON, STATE, STN_ID
import twx
import pandas as pd
...
def insert_data_netcdf_db(db_path, insert_objs):
    ...
    #name = np.array([str(str(row[5]), errors="ignore") for row in all_stn_rows], dtype=np.object)
    name = np.array([str(row[5]) for row in all_stn_rows], dtype=np.object)
...
class InsertGhcn(Insert):
    ...
    def get_stns(self):
        ...
        for line in afile.readlines():
            ...
            if fips_code in self.VALID_FIPS_CODES:
                ...
                name = str(line[41:71].strip(), errors='ignore')
    ...
    def parse_stn_obs(self, stn_id):
            ...
            if element in self.VALID_ELEMENTS:
                ...
                for day in self.MONTH_DAYS:
                    ...
                    try:
                        ...
                        ymd = int("".join([str(year), "%02d" % (month,), "%02d" % (day,)]))
        ...
        obs[:] = [tuple(x) for x in list(obs_dict.values())]
        obs = obs[np.argsort(obs['ymd'])]
...
def add_monthly_means(ds_path, var_name, max_miss=9):
    ...
    if 'time_mth' not in list(ds.variables.keys()):
    ...
    if var_mthly_name not in list(ds.variables.keys()):
    ...
    if var_miss_name not in list(ds.variables.keys()):
...
def add_obs_cnt(ds_path, elem, start_date, end_date, stn_chk=500):
    ...
    if "mth" not in list(ds.dimensions.keys()):
    ...
    if vname not in list(ds.variables.keys()):
