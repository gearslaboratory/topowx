    ...
    def get_stn_obs(self, stnid, year_start_end=None, max_tries=5, nsecs_sleep_tries=15, **kwargs):
        ...
        **kwargs
        ...
        for i, yr in enumerate(yrs):
            ...
            url_args['year'] = yr
            url = "?".join([URL_SNOTEL, urllib.parse.urlencode(url_args)])  
            c.setopt(c.URL, url)
...
def raws_build_stn_metadata(fpath_stnids, fpath_out):
    ...
    for stn_id in afile.readlines():
        ...
        try:
            ...
            response = urllib.request.urlopen("".join([RAWS_STN_TIME_SERIES_URL, stn_id]))
            plines = response.readlines()
            ...
            response = urllib.request.urlopen("".join([RAWS_STN_METADATA_URL, stn_id]))
            plines = response.readlines()
            lon, lat, elev = [None] * 3
...
def _save_dly_series(stn_id, start_date, end_date, out_path):
    ...
    req = urllib.request.Request(STN_TIME_SERIES2_URL, data)
    response = urllib.request.urlopen(req)
