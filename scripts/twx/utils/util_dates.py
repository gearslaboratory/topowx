...
def mdL(date):
    if isinstance(date, cftime.DatetimeGregorian):
        date_datetime = datetime(date.year, date.month, date.day, date.hour, date.minute, date.second)
        date = date_datetime
    try:
        return int(datetime.strftime(date,"%m%d"))
    except ValueError:
        return int("%02d%02d"%(date.month,date.day))
