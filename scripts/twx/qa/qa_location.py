...
def _get_elev_usgs(lon, lat, maxtries):
    ...
    #req = urllib.request.Request(URL_USGS_NED, data)         
    ntries = 0
    user_agent = 'Mozilla/5.0 (X11; Ubuntu; Linux i686; rv:24.0)'\
    ...
    while 1:
        ...
        try:
            ...
            response = requests.get(URL_USGS_NED, params=values, headers=headers)
            #print('TEST')
            break
...
def _get_elev_geonames(lon, lat, usrname_geonames, maxtries):
    ...
    while 1:
        ...
        elev = float(response.text.strip())
