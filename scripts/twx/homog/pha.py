...
from twx.utils import get_mth_metadata, mkdir_p, YEAR, MONTH, YMD, DATE, DAY
...
#def recreate_run(path_out_run, path_out_src, path_src):
#    mkdir_p(path_out_run)

#    shutil.move(os.path.join(path_out_src, "testdata"), path_out_run)

 #   fpath_bench_tar = os.path.join(path_out_run, "testdata", "benchmark.tar.gz")

  #  os.chdir(os.path.join(path_out_run, "testdata"))

   # subprocess.call(["tar", "-xzvf", fpath_bench_tar, '-C', '.'])
...
def setup_pha(fpath_pha_tar, path_out_src, path_out_run, yr_begin, yr_end, stns,
    ...
    print("Compiling PHA...")
    # Adriano Changes
    replacement('Makefile',"rmdir $(INSTALLDIR)","rm -rf $(INSTALLDIR)")
    replacement('./scripts/source_compiles/PHAv52i.sh',"#!/bin/sh","#!/bin/bash")  
    replacement('./scripts/source_compiles/PHAv52i.sh',"gawk","awk") 
    replacement('./scripts/source_compiles/compile.sh',"#!/bin/sh","#!/bin/bash") 
...
def run_pha(path_src, path_run, varname):
    '''
    Run a PHA instance
...
def load_snotel_sensor_hist(station_ids=None):
    ...
    stnhist['date_new_sensor'] = pd.to_datetime(stnhist['date_new_sensor'])
        ...
        #stnhist = stnhist.reindex(columns = station_ids).dropna()
